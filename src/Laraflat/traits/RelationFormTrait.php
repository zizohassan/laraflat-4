...
trait RelationFormTrait
{
    ...
    protected function appendInput($relation , $name ){
        ...
        if($relation->input_type == 'select'){
            return '@include("laraflat::fileds.php.select" , [  "label"  => trans("'.$info['label'].'") , "array" => '.$info['array'].'  , "value" => $row->' . $name . ' ?? null'.'  ,  "name" =>"' . $name . '" ])'."\n";
        }elseif ($relation->input_type == 'checkbox'){
            return '@include("laraflat::fileds.php.checkbox" , [  "label"  => trans("'.$info['label'].'") , "array" => '.$info['array'].'   , "selectedArray" => $row->' . $name . ' ?? null'.'  ,  "name" =>"' . $name . '" ])'."\n";
        }
    }
    ...
    protected function appendInputManyToMany($relation , $name  , $module){
        ...
        if($relation->input_type == 'select'){
            return '@include("laraflat::fileds.php.select" , [  "label"  => trans("'.$info['label'].'") , "array" => '.$info['array'].'  , "value" => $row->' . $module . $pluck. ' ?? null'.'  ,  "name" =>"' . $name . '[]" , "multiSelect" => true ])'."\n";
        }elseif ($relation->input_type == 'checkbox'){
            return '@include("laraflat::fileds.php.checkbox" , [  "label"  => trans("'.$info['label'].'") , "array" => '.$info['array'].'   , "selectedArray" => $row->' . $module .$pluck . ' ?? null'.'  ,  "name" =>"' . $name . '[]" ])'."\n";
        }
    }
